...
public class BaseActivityValidator<T, TDto> : AbstractValidator<T> where TDto : BaseActivityDto
{
    ...
    {
        RuleFor(x => selector(x).Title).NotEmpty().WithMessage("Title is required!").MaximumLength(100).WithMessage("Title must not exceed 100 charachters");
        RuleFor(x => selector(x).Description).NotEmpty().WithMessage("Title is required!");
